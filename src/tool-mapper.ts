...
function cleanJsonSchema(schema: JsonSchemaObject): JsonSchemaObject {
  ...
  // Remove 'const' - Gemini doesn't support it, convert to enum with single value
  if ("const" in cleaned) {
    const constValue = cleaned.const;
    delete cleaned.const;
    // Convert const to enum with single value for compatibility
  ...
  // Clean arrays - handle both camelCase (JSON Schema) and snake_case (Gemini)
  for (const key of [
    "allOf",
    "anyOf",
    "oneOf",
